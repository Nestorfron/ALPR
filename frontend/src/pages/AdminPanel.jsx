...
const AdminPanel = () => {
  const { usuario, jefaturas, loading, token } = useAppContext();
      ...
      <main className="flex-1 px-6 py-8 space-y-8 mb-8">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Rol: {usuario.rol_jerarquico === "ADMINISTRADOR" ? "Administrador" : "Acceso limitado"}
          </p>
        </div>
        ...
        {usuario.rol_jerarquico === "ADMINISTRADOR" ? (
          jefaturas && jefaturas.length > 0 ? (
                  ...
                  <div key={jefatura.id} className="space-y-4">
                    ...
                    {jefatura.zonas && jefatura.zonas.length > 0 ? (
                            ...
                            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-blue-100 dark:border-slate-700 overflow-x-auto">
                              <div className="flex px-4 py-3 bg-blue-50 dark:bg-slate-900 border-b border-blue-100 dark:border-slate-700 rounded-t-2xl justify-between items-center">
                                ...
                                <IconButton
                                  ...
                                  onClick={() => navigate(`/crear-dependencia/${zona.id}`)}
                                  size="sm"
                                />
                              </div>

                              {/* Dependencias */}
                              {zona.dependencias && zona.dependencias.length > 0 ? (
                                        ...
                                        >
                                          ...
                                          <td className="px-4 py-2 text-sm text-gray-700 dark:text-gray-300">
                                            {(() => {
                                              ...
                                              return jefe ? `G${jefe.grado || ""} ${jefe.nombre}` : "Sin jefe";
                                            })()}
                                          </td>
                                          <td className="px-4 py-2 text-sm text-gray-700 dark:text-gray-300">
                                            {dep.usuarios?.filter(
                                          ...
                                          <td className="flex items-center justify-center gap-2 px-4 py-2">
                                            <IconButton
                                              ...
                                              onClick={() => alert(`Editar ${dep.nombre}`)}
                                              size="sm"
                                            />
                                            <IconButton
                                              icon={Plus}
                                ...
                                <div className="p-4 text-center">
                                  <p className="text-gray-600 dark:text-gray-400">No hay dependencias.</p>
                                </div>
                              )}
                            </div>
                          </div>
                      ...
                      <div className="flex items-center justify-center bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-blue-100 dark:border-slate-700 p-4 text-center">
                        <p className="text-gray-600 dark:text-gray-400">No hay zonas.</p>
                        <IconButton
                          icon={PlusCircle}
                          tooltip="Agregar zona"
                          onClick={() => navigate(`/crear-zona/${jefatura.id}`)}
            ...
            <div className="flex items-center justify-center">
              <p className="text-gray-600 dark:text-gray-400">No hay jefaturas.</p>
              <IconButton
                icon={PlusCircle}
